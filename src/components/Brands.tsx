      ...
      <div className="relative flex">
        ...
        >
          {[...items, ...items].map((brand, i) => (
            <span key={i} className="text-4xl md:text-6xl font-extrabold text-zinc-200 dark:text-white/[0.04] uppercase tracking-tighter hover:text-brand transition-all duration-700 cursor-default select-none font-display italic">
              {brand}
            </span>
          ))}
        </motion.div>
