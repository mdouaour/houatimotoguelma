    ...
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      ...
      >
        <svg className="absolute inset-4 w-[calc(100%-32px)] h-[calc(100%-32px)]" aria-hidden>
          {/* Outer dashed track — circuit trace */}
          <motion.rect
          ...
          />
          {/* Corner nodes — circuit junction dots */}
          {[[24,24],[24,"calc(100%-24px)"],["calc(100%-24px)",24],["calc(100%-24px)","calc(100%-24px)"]].map((pos,i) => (
            <motion.g key={i}>
              <circle cx={pos[0]} cy={pos[1]} r="4" fill="#00FF88" opacity="0.15" />
      ...
      >
        <div className="w-full h-full rounded-full bg-gradient-radial from-green-500/10 via-transparent to-transparent blur-2xl" />
      </motion.div>

      {/* Scooter + rider — dynamic suspension bounce */}
      <motion.div
        className="absolute bottom-[12%] right-[6%]"
        style={{
          width: "min(120px, 18vw)",
          height: "min(110px, 16vw)",
          filter: "drop-shadow(0 0 20px rgba(0,255,136,0.08))",
        }}
        animate={{
          y: [0, -18, -4, -22, 0],
          x: [0, 12, -4, -10, 0],
