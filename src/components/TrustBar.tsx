  ...
  return (
    <section className="py-12 bg-surface border-y border-border-subtle relative z-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <StaggerReveal className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12" staggerDelay={0.1}>
          <RevealItem><TrustItem icon={<Clock />} text={t.trust.exp} label="Precision_Time" /></RevealItem>
          <RevealItem><TrustItem icon={<ShieldCheck />} text={t.trust.parts} label="Genuine_Guaranteed" /></RevealItem>
          <RevealItem><TrustItem icon={<Wrench />} text={t.trust.service} label="Technical_Excellence" /></RevealItem>
        </StaggerReveal>
      </div>
    </section>
  );
